/* ===========================================================
   PIXORA PRO — BUILD 004
   Namespace: PixoraApp
   Feature: Pinch-to-zoom (2-finger gesture) replaces
            the old Zoom In / Zoom Out buttons.
   =========================================================== */

const PixoraApp = {

  /* -----------------------------------------------------------
     Central config — avoids "magic numbers" scattered in code.
     Change zoom limits here only, nowhere else.
  ----------------------------------------------------------- */
  config: {
    MIN_ZOOM: 0.5,
    MAX_ZOOM: 5
  ...
  /* -----------------------------------------------------------
     Pinch-to-zoom internal tracking state
     (kept separate from app "state" since it's gesture-only data)
  ----------------------------------------------------------- */
  pinch: {
  ...
  /* ===========================================================
     BUILD 004 — Pinch-to-Zoom (2-finger gesture)
     Replaces the old Zoom In / Zoom Out buttons.
     =========================================================== */

  bindPinchZoomEvents() {
    const wrapper = this.elements.canvasWrapper;
    wrapper.addEventListener('pointerdown', (e) => this.handlePinchPointerDown(e));
    wrapper.addEventListener('pointermove', (e) => this.handlePinchPointerMove(e));
    wrapper.addEventListener('pointerup', (e) => this.handlePinchPointerEnd(e));
    wrapper.addEventListener('pointercancel', (e) => this.handlePinchPointerEnd(e));
    wrapper.addEventListener('pointerleave', (e) => this.handlePinchPointerEnd(e));
  },

  isPinchAllowed() {
    // Pinch-zoom should not run while cropping or adjusting filters,
    // same rule the old zoom buttons followed.
    return this.state.imageLoaded && !this.state.cropMode && !this.state.filterMode;
  },

  handlePinchPointerDown(e) {
    if (!this.isPinchAllowed()) return;

    this.pinch.activePointers.set(e.pointerId, { x: e.clientX, y: e.clientY });

    if (this.pinch.activePointers.size === 2) {
      this.pinch.startDistance = this.getPinchDistance();
      this.pinch.startZoom = this.state.zoomLevel;
    }
  },

  handlePinchPointerMove(e) {
    if (!this.isPinchAllowed()) return;
    if (!this.pinch.activePointers.has(e.pointerId)) return;

    this.pinch.activePointers.set(e.pointerId, { x: e.clientX, y: e.clientY });

    if (this.pinch.activePointers.size === 2) {
      const currentDistance = this.getPinchDistance();
      if (this.pinch.startDistance > 0) {
        const scaleFactor = currentDistance / this.pinch.startDistance;
        this.applyZoom(this.pinch.startZoom * scaleFactor);
      }
    }
  },

  handlePinchPointerEnd(e) {
    this.pinch.activePointers.delete(e.pointerId);

    // Fewer than 2 fingers left -> gesture ends, reset baseline
    if (this.pinch.activePointers.size < 2) {
  ...
  getPinchDistance() {
    const points = Array.from(this.pinch.activePointers.values());
    const [p1, p2] = points;
    const dx = p2.x - p1.x;
    const dy = p2.y - p1.y;
  ...
  applyZoom(newZoom) {
    ...
    this.state.zoomLevel = Math.max(MIN_ZOOM, Math.min(newZoom, MAX_ZOOM));
